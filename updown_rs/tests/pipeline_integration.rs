...
use updown::theory::optimizer::{verify_falsifiability, OptimizerConfig};
...
fn test_full_pipeline_with_text() {
    ...
    assert!(!p1.s.vertices.is_empty(), "Phase 1 应有节点");
    assert!(p1.max_depth > 0.0, "Phase 1 应有深度");
    assert!(
        !p1.reversibility_record.is_empty(),
        "Phase 1 应有可逆性记录"
    );
    ...
    assert!(
        report.words_fully_recoverable,
        "词应完全可逆: rate={:.2}",
...
fn test_falsifiability_criterion() {
    // 在不同数据集上验证可证伪判据
    let texts = ["苹果香蕉水果", "基因演化自然选择", "信息熵守恒定律"];
...
fn test_ode_convergence() {
    ...
    use updown::theory::function_families::{FamilyParams, FunctionFamily};
