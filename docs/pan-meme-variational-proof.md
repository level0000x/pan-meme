...
对社区 $X_i$，$|X_i| = n_i$。其导出子图 $G[X_i]$ 的归一化拉普拉斯 $\mathcal{L}^{(i)}$ 的特征值为 $\{\lambda_k^{(i)}\}$，热迹为 $\Theta_i(t)$。记 $\beta_0^{(i)} = \lim_{t\to\infty} \Theta_i(t)$——社区 $X_i$ 导出子图中连通分量的个数（$\beta_0^{(i)} \ge 1$，连通社区取等号）。
...
对任何 $M^{(k)}$，其更新步 $M^{(k+1)} = N(M^{(k)})$ 满足 $\|M^{(k+1)} - M^*\| \le \rho \|M^{(k)} - M^*\|$ 对某个 $\rho < 1$（Banach 不动点定理——$\rho(J_N(M^*)) < 1$ 保证 N 在 $M^*$ 邻域内为压缩映射，数值验证见 §B）。N 迭代的收敛性由此得证：$M^{(k)} \to M^*$ 且 $\|\Delta M^{(k)}\| \to 0$。

在不动点处 $\Phi(M^*) - \Phi(M^*) = 0$。沿迭代轨迹，考虑差分 $\Delta\Phi^{(k)} = \Phi_i(M^{(k+1)}) - \Phi_i(M^{(k)})$。因 $M^*$ 是 $\Phi$ 沿 N 迭代的严格局部极小值（$\Phi(M^*) < \Phi(M)$ 对 $M \neq M^*$ 在邻域中——由 $\rho(J_N) < 1$ 指数吸引子性质和 $\Phi$ 在 $M^*$ 处的正定 Hessian 保证），$\Delta\Phi^{(k)} \le 0$ 对充分大的 $k$ 成立，且 $\Delta\Phi^{(k)} \to 0$ 当 $k \to \infty$。综合起来 $\Phi(M^{(k)})$ 单调递减至 $\Phi(M^*)$，即 $\dot\Phi \le 0$ 的离散类比。$\square$
