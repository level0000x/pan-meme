...
| $\alpha_2, \delta_1$ | $(0,1]$ | $\Theta_{0.5} \in [\beta_0+1, n]$ |
...
*证明纲要*。$D_i$ 和 $S_i$ 均由热迹定义。在 N 算子迭代中，$\Phi(M) = n \cdot D \cdot (1-S)$（省略社区下标）。N 迭代的收敛性由 $\rho(J_N(M^*)) < 1$ 保证（§C），这意味着 $M^{(k)} \to M^*$ 且 $\|\Delta M^{(k)}\| \to 0$。在不动点处 $\dot\Phi(M^*) = \nabla\Phi(M^*)^\top \Delta M = 0$（一阶条件下 $\Delta M^* = 0$）。沿迭代轨迹的差分 $\Delta\Phi^{(k)} = \Phi(M^{(k+1)}) - \Phi(M^{(k)})$ 在远离 $M^*$ 处为负（N 算子的梯度下降性质），在 $M^*$ 处渐近趋零。综合起来 $\Phi(M^{(k)})$ 单调递减至 $\Phi(M^*)$，即 $\dot\Phi \le 0$ 的离散类比。$\square$
