...
*证明*。三步链式论证。(i) FCA 蕴涵 → 闭包重叠 → 高边权（定义 4）。(ii) Davis-Kahan 定理（von Luxburg 2007, Thm 8.1.14）：$\|U_i - U_j\|_2 \le 2\sqrt{2}/\lambda_{k^*+1} \cdot \|A_i - A_j\|_F$，其中 $A_i, A_j$ 为邻接矩阵的行——高边权使行向量接近，从而谱嵌入坐标接近。(iii) 约束矩阵 $Q$ 的 must-link 强制 → 同社区。$\square$
...
| $\alpha_2, \delta_1$ | $(0,1]$ | $\Theta_{0.5} \in (\beta_0+1, n]$（$\Theta$ 严格递减，$t^*/2 < t^* \Rightarrow \Theta(t^*/2) > \Theta(t^*) = \beta_0+1$） |
...
*证明纲要*。$D_i$ 和 $S_i$ 均由热迹定义。在 N 算子迭代中，$\Phi(M) = n \cdot D \cdot (1-S)$（省略社区下标）。N 迭代的收敛性由 $\rho(J_N(M^*)) < 1$ 保证（数值验证见 §C），这意味着 $M^{(k)} \to M^*$ 且 $\|\Delta M^{(k)}\| \to 0$。在不动点处 $\Delta\Phi(M^*) = \Phi(M^*) - \Phi(M^*) = 0$。沿迭代轨迹的差分 $\Delta\Phi^{(k)} = \Phi(M^{(k+1)}) - \Phi(M^{(k)})$ 在远离 $M^*$ 处为负（N 算子的梯度下降性质），在 $M^*$ 处渐近趋零。综合起来 $\Phi(M^{(k)})$ 单调递减至 $\Phi(M^*)$，即 $\dot\Phi \le 0$ 的离散类比。$\square$

（该定理的连续极限版本在定理 4 的变分框架中自动满足——$\dot E \le 0$ 蕴含 $\dot\Phi \le 0$。参见 §3.5。）
...
在约束流形上 $F(M)=0$ 由拉格朗日乘子 $\lambda$ 精确维持。约束反力 $J_F^\top \lambda$ 垂直于 $\mathcal{M}$ 的切空间，故 $J_F \dot M = 0$（$F$ 沿轨迹恒为零——理想完整约束的标准性质，Goldstein 1980 §2.4）。因此：
...
**定理 6（稳态存在性与唯一性）**。测地线的终点满足 $\dot M = 0$ 和 $F(M^*) = \mathbf{0}$。在正参数域下，该稳态存在（$N$ 连续映射 $[0,1]^5 \to (0,1)^5 \subseteq [0,1]^5$，Brouwer 不动点定理应用于 $[0,1]^5$），在非退化参数下唯一（隐函数定理 + Jacobian 满秩），退化时由典范初值选定。
