...
1. **深度单调性**：$D$ 是社区规模 $n_i$ 的严格递减函数——社区越大，每顶点归一化热迹越小。$C_i \preceq C_j$ 下，$|A_i| \le |A_j|$（外延更小）但 $|B_i| \ge |B_j|$（内涵更大）——两股力对抗。$D_i$ 与 $D_j$ 的大小关系取决于 Galois 连接中哪一侧主导，非普遍的单调方向。深度单调性的精确方向需分情形讨论（见证明）。
2. **广度单调性**：$B_i \le B_j$。子概念的全局广度不大于父概念——$\Theta_G$ 严格递减，$|B_i| \ge |B_j| \Rightarrow$ 子概念词集更大，在（由 $|A_i| \le |A_j|$ 决定的更小）评估时间 $\tau_i$ 处，热迹 $\Theta_G(\tau_i) \ge \Theta_G(\tau_j)$（严格递减），导致 $B_i \le B_j$。父概念因涵括更广而在全局热迹中占据更大份额。
...
对混合类型社区（含字符和词顶点），$G[X_i]$ 为二部块结构，$f$ 的显式形式更复杂，但 $D \downarrow$ 作为 $n_i \uparrow$ 的总体趋势在数值实验中一致成立（§C）。$\square$

**定义 11（偏序约束集 $\mathcal{C}_{\text{order}}$）**。概念格 $\mathfrak{B}(\mathbb{K})$ 的 Hasse 图中的所有覆盖关系 $(C_i, C_j)$ 对应的不等式约束——不等号方向由 $f$ 的递减性结合 $|X_i|$ 的相对大小决定，非由 $\preceq$ 直接给出：

$$\mathcal{C}_{\text{order}} = \{g_{D,ij}(M) = D_j - D_i \le 0 \mid |X_i| \le |X_j|, \; C_i, C_j \text{ 在 Hasse 图中可比}\} \cup \{g_{B,ij}(M) = B_i - B_j \le 0 \mid |X_i| \le |X_j|, \; C_i, C_j \text{ 在 Hasse 图中可比}\}$$

约束的关键是**哪对社区可比**（由 $\preceq$ 回答）——方向的最终确定由 $|X_i|$（社区规模）驱动，而非 $\preceq$。在 N 算子迭代中，$|X_i|$ 先由聚类确定，随后 $D,B$ 自然满足对应的单调性。（注：$|X_i| \le |X_j| \Rightarrow D_i \ge D_j \Rightarrow D_j - D_i \le 0$；$|X_i| \le |X_j| \Rightarrow B_i \le B_j \Rightarrow B_i - B_j \le 0$。）
...
在约束流形附近 $F(M) \approx \mathbf{0}$，故 $J_F \dot M \approx 0$（$F$ 沿轨迹缓变）→ $\dot M^\top J_F^\top \lambda \approx 0$。因此：
