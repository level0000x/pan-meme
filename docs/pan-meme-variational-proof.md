...
$$L(M, \dot M) = \underbrace{\frac12 \|\dot M\|^2}_{\text{动能}} - \underbrace{\Phi(M)}_{\text{"势能"——$\Phi$ 为非保守耗散泛函，在 $L$ 中借位势能角色；系统无孤立能量守恒}}$
...
$V(M) = \|M - M^*\|_2^2$ 沿此线性化动力学的导数为 $\dot V = -2(M-M^*)^\top \frac{\mathcal{D} + \mathcal{D}^\top}{2} (M-M^*)$。$\frac{\mathcal{D}+\mathcal{D}^\top}{2}$ 是 $\mathcal{D}$ 的对称部分，正定（因 $\mathcal{D}$ 的特征值均有正实部——标准 Lyapunov 定理保证存在对称正定 $P$ 使 $P\mathcal{D} + \mathcal{D}^\top P = I$；$\frac{\mathcal{D}+\mathcal{D}^\top}{2}$ 不一定正定——因为 $P$ 一般不是单位矩阵。在标准参数下数值成立，但非定理保证——此处诚实标注为数值验证。）
