...
$$L(M, \dot M) = \underbrace{\frac12 \|\dot M\|^2}_{\text{动能}} - \underbrace{\Phi(M)}_{\text{势能（耗散泛函）}}$$

**注：关于自洽约束**。$F(M) = M - N(M) = \mathbf{0}$ 是不动点方程——它不是标准力学完整约束（如粒子在球面上），而是自洽性条件。经典力学中的完整约束 $f(q) = 0$ 定义了位形空间中的曲面，约束反力 $J_f^\top \lambda$ 垂直作用于该曲面；这里的 $F(M) = \mathbf{0}$ 的解集是孤立点 $\{M^*\}$，不能直接套用完整约束框架。本文将 $F(M)$ 处理为**乘法罚函数**——在拉格朗日量中添加 $\lambda^\top F(M)$ 作为渐近惩罚项，$\lambda$ 为罚权重——而非标准约束乘子。不等式约束 $g_l(M) \le 0$ 定义可行区域 $\mathcal{F}$，在变分原理中作为路径的容许条件——不进入拉格朗日量内部（标准约束变分惯例，Gelfand & Fomin 1963, §12）。
...
**过阻尼极限**。在信息传播中惯性项 $\ddot M$ 可忽略（无"弹跳"——状态平滑趋近）。取 $\varepsilon \to 0$ 极限（Tikhonov 定理，Verhulst 2005 §15）：

$$\Gamma \dot M = -\nabla\Phi(M)$$

状态以 $\Gamma$ 的逆时间常数沿 $\nabla\Phi$ 的负方向流动。

**不动点的吸引性**。该梯度流的不动点满足 $\nabla\Phi(M) = \mathbf{0} \iff M = (0, *, *, *, 1)$ 或 $M = (1, *, *, *, 0)$——即 $D=0,S=1$ 或 $D=1,S=0$。然而在 N 算子的耦合动力学中，这些点均非可达（定理 D + §3.5 的 $\Phi$ 物理角色分析）。**N 迭代的不动点 $M^* = N(M^*)$ 不是梯度流 $\Gamma \dot M = -\nabla\Phi$ 的不动点**——它是自洽动力学 $M^{(k+1)} = N(M^{(k)})$ 的指数吸引子。梯度流 $\Gamma \dot M = -\nabla\Phi$ 是概念框架（拉格朗日力学提供了变分语言和能量分析），而实际轨迹由 N 迭代确定——两套动力学在定性上共享"向稳态弛豫"的行为，但在不动点处不重合。$\square$
