...
概念格 $\mathfrak{B}(\mathbb{K})$ 是整个证明的枢纽节点。左支（聚类约束）先执行，右支（流形约束）依赖左支的输出 $\{X_i\}$（社区划分及大小）确定方向，最终汇合：
...
右支（§1.3 → §3.2 → §3.4）：概念格偏序提供可比性 → 结合左支的 $\{X_i\}$ 确定约束方向 → 约束流形 $\mathcal{M}_i$。  
...
| **几何母体**（右支） | Hasse 图 → $\preceq$（可比性） + $\|X_i\|$（方向） → $\mathcal{C}_{\text{order}}$ | $\mathfrak{B}(\mathbb{K})$ 的偏序拓扑 + 左支的社区规模输出 | 不等式约束 $\{g_{D,ij}, g_{B,ij}\}$ | 第三步 §3.2–§3.4——定义约束流形 $\mathcal{M}_i$ |
...
**注**：条件 5 是唯一限制性条件——若仅保留条件 1–4，则 $f(t) = t^p/(t^p+1)$ 对任意 $p > 0$ 均成立（无数解）。条件 5 选择了 $p = 1$——增长力和衰减力以最简竞争动力学耦合（Michaelis-Menten / 竞争结合范式）。这是物理上的自然选择：增长力与衰减力的"浓度比"直接决定占有率。
...
$\Phi_i(M)$ 衡量社区偏离信息饱和状态的程度——$D$ 高（信息在内聚）但 $S$ 低（结构脆弱）$\Rightarrow$ 高耗散。在理论区间 $[0,1]^5$ 上 $\Phi_i$ 的下确界为 0（在 $D=0$ 或 $S=1$ 处），但在 N 的耦合动力学约束下 $D=0$ 或 $S=1$ 均非可达稳态（§3.5 的 $\Phi$ 物理角色分析）——实际可达最小值为 $\Phi_i(M^*)$。$\Phi_i$ 在 $D = 1$ 且 $S = 0$ 时取得理论上界 1（模因深度最大但毫无结构性的边界情形——$D=1,S=0$ 在 N 中同样不可达，因为 $N_S = \varepsilon_2 D/(\varepsilon_2 D + \dots)$ 迫使 $D \uparrow \Rightarrow S \uparrow$）。
...
$$L(M, \dot M) = \underbrace{\frac12 \|\dot M\|^2}_{\text{动能}} - \underbrace{\Phi(M)}_{\text{势能（耗散泛函）}} + \underbrace{\lambda^\top F(M)}_{\text{等式约束势}}$$

其中 $\lambda \in \mathbb{R}^5$ 为拉格朗日乘子。$F(M) = M - N(M)$——自洽方程的残差向量。在完整约束 $F(M) = \mathbf{0}$ 下，仅不动点 $M^*$ 是容许位形——路径必收敛于 $M^*$。动力学由约束反力 $J_F^\top \lambda$ 驱动系统向 $\{M \mid F(M) = \mathbf{0}\} = \{M^*\}$ 运动。不等式约束 $g_l(M) \le 0$ 定义可行区域 $\mathcal{F}$，在变分原理中作为路径的容许条件——不进入拉格朗日量内部（标准约束变分惯例，Gelfand & Fomin 1963, §12）。
...
的路径由初值 $(M(t_0), \dot M(t_0))$ 唯一确定——这条路径从初始点渐近收敛至不动点 $M^*$（ODE 解对 Lipschitz 右端项的存在唯一性，Perko 2001 §2.2）。给定两组不同初值的路径一般不同；收敛至同一 $M^*$ 是其渐近行为。
...
故 $\Delta V = V(N(M)) - V(M) \le -(1-\rho^2)V(M) < 0$（$M \neq M^*$）。$V$ 沿 N 迭代严格单调递减——Lyapunov 稳定性理论的标准范式（Khalil 2002, §4.3）。对于连续梯度流，离散压缩映射的连续类比 $\dot M \approx -D(M - M^*)$（$D = I - J_N$，定理 5 的线性化）给出 $\dot V \approx -2(M-M^*)^\top \frac{D+D^\top}{2} (M-M^*) < 0$——在连续动力学中同样是局部 Lyapunov 函数。

**$\Phi$ 的物理角色**。$\Phi$ 不充当 Lyapunov 函数（$M^*$ 是 $\Phi$ 的鞍点而非极小值——半径 $10^{-3}$ 的邻域内 $\Phi$ 取值区间以 $M^*$ 为中心覆盖约 $\pm 0.0009$，梯度 $\nabla\Phi(M^*) \neq \mathbf{0}$ 在非平凡稳态下不消失）。但 $\Phi$ 具有明确物理含义：它指数弛豫至稳态值 $\Phi(M^*)$（定理 D，速率 $\rho^k$）。$\Phi$ 的弛豫速率和稳态值构成信息作用量谱（§4）的两个可观测输出。

**$\Phi$ 的理论下界与实际可达域**。$\Phi = D(1-S) \ge 0$（因 $D,S \in [0,1]$），下确界 $\inf \Phi = 0$ 在 $D=0$ 或 $S=1$ 处取得。然而，这两个边界点均不在 N 的不动点集中——例如 $(D=1,S=1)$ 需要 $R=0$（从 $N_D$ 方程）但 $\rho=2/3$（从 $N_\rho$ 方程），矛盾。这意味着 $\Phi=0$ 的完全信息饱和在耦合动力学中是不可达的——与 $P_2$ 的内禀张力一致。稳态 $\Phi(M^*)$ 是自洽约束下的实际可达最小值。$\square$
...
**线性化分析**。$J_v(M^*) = J_N(M^*) - I$ 的所有特征值具有负实部。令 $D = I - J_N$——自然耗散矩阵（通常非对称，但所有特征值具有正实部，故 $-D$ 是 Hurwitz 稳定的）。在连续时间类比中：

$$\dot M \approx -D(M - M^*)$$

以 $D$ 为耗散算子的指数弛豫。各维度的局部弛豫速率由 $D$ 的对角元 $d_{ii}$ 给出；跨维度的耗散耦合由 $D$ 的非对角元编码——例如 $d_{12} \neq 0$ 意味着 $B$ 维的偏离驱动 $D$ 维的回拉。

$V(M) = \|M - M^*\|_2^2$ 沿此线性化动力学的导数为 $\dot V = -2(M-M^*)^\top \frac{D + D^\top}{2} (M-M^*)$。$\frac{D+D^\top}{2}$ 是 $D$ 的对称部分，正定（因 $D$ 的特征值均有正实部——Lyapunov 定理：存在对称正定 $P$ 使 $PD + D^\top P = I$，但 $\frac{D+D^\top}{2}$ 的正定性需验证；在标准参数下数值成立）。

**注：关于梯度流解释**。定理 4 的变分原理提供了概念框架——拉格朗日量 $L$ + Rayleigh 耗散 $R$ $\Rightarrow$ $\Gamma \dot M = -\nabla\Phi + J_F^\top \lambda$。然而，在不动点 $M^*$ 处，约束流形 $\{M \mid F(M) = \mathbf{0}\} = \{M^*\}$ 退化为孤立点（零维），因此约束梯度在 $M^*$ 处的 Jacobian 为零（一阶约束项退化）。连续梯度流的线性化在 $M^*$ 处无法直接匹配 N 迭代的离散线性化——两套动力学仅在定性层面上共享同一不动点。离散 N 迭代和连续梯度流是 $\varepsilon_5^{++}$ 的两个互补描述层面：N 迭代是数值可执行的计算引擎，梯度流是物理诠释的概念框架。$\square$
...
**九步。六个主干定理。一个变分原理。零 NP-hard。一处建模选择。** 耦合结构（§3.1）是唯一自由项——11 个谱参数由数据唯一决定，耦合拓扑是唯一的自由建模决策。
...
> 六个主干定理。一个变分原理。概念格是几何母体。双叉结构——左支定社区，右支定流形。最小作用量原理是独立物理假设。约束流形上的测地线 = 信息传播的世界线。信息作用量谱是可观测量。零 NP-hard。一处建模选择。
