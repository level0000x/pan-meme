...
| $\alpha_2, \delta_1$ | $((\beta_0+1)/n,\; 1)$ | $\Theta_{0.5} \in (\beta_0+1, n]$（$\Theta$ 严格递减，$t^*/2 < t^* \Rightarrow \Theta(t^*/2) > \Theta(t^*) = \beta_0+1$） |
...
在约束流形上 $F(M)=0$ 由拉格朗日乘子 $\lambda$ 精确维持。约束反力 $J_F^\top \lambda$ 垂直于 $\mathcal{M}$ 的切空间，故在流形上 $J_F \dot M = 0$（$F$ 沿流形上的轨迹恒为零——理想完整约束的标准性质，Goldstein 1980 §2.4）。

在实际的物理轨迹中，系统从 $M(t_0) \notin \mathcal{M}$ 出发并渐近收敛至 $M^* \in \mathcal{M}$。对于有限时间 $t < \infty$，$F(M(t)) \neq 0$，因此 $J_F \dot M \neq 0$ 且 $\dot M^\top J_F^\top \lambda \neq 0$——约束反力做功直到系统到达流形。能量演化方程严格形式为：
...
能量渐近单调递减——阻尼持续耗散动能和势能，系统趋向静止于不动点。
