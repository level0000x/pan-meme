...
(ii) 深度单调性。$C_i \preceq C_j \Rightarrow A_i \subseteq A_j \Rightarrow n_i \le n_j$。因此 $\tau_i = n_i/n \le n_j/n = \tau_j$——子概念的热迹在更早的时间点上被评估。$G[X_i]$ 是 $G[X_j]$ 的导出子图。需要比较 $\Theta_i(\tau_i)/n_i$ 与 $\Theta_j(\tau_j)/n_j$。

深度单调性 $D_i \le D_j$ 的严格证明需克服两个非平凡障碍：(a) 归一化拉普拉斯在导出子图上不满足标准交错定理——即使 $G$ 是正则图，导出子图 $G[X_i]$ 一般不是正则图，$D^{-1/2}WD^{-1/2}$ 的导出子图特征值不等于原特征值的交错（Chung 1997, §1.5 将其列为开放问题）；(b) 即使 $\Theta_i(t) \le \Theta_j(t)$ 对所有 $t$ 成立（例如在非归一化拉普拉斯的情形下），由于 $\Theta$ 递减且 $\tau_i \le \tau_j$，$\Theta_j(\tau_i) \ge \Theta_j(\tau_j)$，无法从 $\Theta_i(\tau_i)/n_i \le \Theta_j(\tau_i)/n_j$ 推出 $\le \Theta_j(\tau_j)/n_j$——最后一个不等号方向相反。

因此，深度单调性**降级为物理猜想**，受以下支撑：(a) 随机图模型上 $\mathbb{E}[D_i] \le \mathbb{E}[D_j]$ 的概率论证（Molloy & Reed 2002 度序列框架）；(b) 大规模数值实验验证（参见 §C）；(c) 正则图 + 非归一化拉普拉斯情形的严格证明作为特例。定理 C 的其余两条（广度单调性、跨度守恒）为严格证明。
...
在 $\mathcal{M}$ 上 $J_F\dot M = 0$，且 $\Gamma^{-1} \succ 0$。若 $\nabla\Phi$ 与 $\ker(J_F)$ 的交角在 $M^*$ 附近有界（LICQ 条件保证），则 $\nabla\Phi^\top \Gamma^{-1} \nabla\Phi \ge \kappa \|\nabla\Phi\|^2$ 对某 $\kappa > 0$ 成立，从而 $\dot\Phi \le -\kappa \|\nabla\Phi\|^2 \le 0$。此即过阻尼极限下的耗散不等式。

完整意义上 $\dot\Phi \le 0$ 对所有时间尺度的严格解析证明要求对 $\Phi$ 沿 N 迭代轨迹作差分符号分析，与 §C 的 Jacobian 结构协调——这是正在进行的完善工作。当前状态：**过阻尼极限下解析证明成立；全阻尼情形受数值证据强力支持**。$\square$
...
| **能量/耗散** | 数值佐证（自正则化） | **定理 D**——耗散不等式 $\dot{\Phi} \le 0$（过阻尼极限下解析证明；全阻尼数值支持） |
