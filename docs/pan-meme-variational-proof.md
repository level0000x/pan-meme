...
$\Phi_i(M)$ 衡量社区偏离信息饱和状态的程度——$D$ 高（信息在内聚）但 $S$ 低（结构脆弱）$\Rightarrow$ 高耗散。在理论区间 $[0,1]^5$ 上 $\Phi_i$ 的下确界为 0（在 $D=0$ 或 $S=1$ 处），但在 N 的耦合动力学约束下 $D=0$ 或 $S=1$ 均非可达稳态（§3.5 的 $\Phi$ 物理角色分析）——实际可达最小值为 $\Phi_i(M^*)$。在完全碎片化极限 $D \to 0, S \to 0$ 时取得最大值。
...
**九步。六个主干定理。一个变分原理。零 NP-hard。零自由参数。** 建模选择仅一处——耦合结构（§3.1）。其余全部由数学从 $\mathfrak{B}(\mathbb{K})$ 的双叉结构强制导出。11 个谱参数由数据唯一决定；耦合拓扑是唯一的自由建模决策。
