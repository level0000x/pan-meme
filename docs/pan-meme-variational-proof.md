...
**定理层级说明**。本文共 10 个定理，分为两层：**主干定理**（1–6）构成闭合链的骨架——有限收敛、热迹渐近、天然归一化、耗散变分原理、N 迭代对应、稳态存在唯一性；**辅助定理**（A, B, C, D）为各步提供技术支撑——约束谱聚类等价性（A）、FCA 一致性（B）、偏序单调性（C）、N 迭代离散耗散不等式（D）。正文中"六个主干定理"指定理 1–6。
...
**定理 D（离散耗散不等式）**。沿 N 算子迭代生成的离散传播序列 $\{M^{(k)}\}$，$\Phi_i(M^{(k)})$ 在不动点 $M^*$ 的邻域 $U$ 内单调递减：$\Phi_i(M^{(k+1)}) \le \Phi_i(M^{(k)})$ 对进入 $U$ 的所有迭代步成立，且 $\Phi_i(M^{(k)}) \to \Phi_i(M^*)$——即 $\dot\Phi \le 0$ 的离散类比。

*证明纲要*。考虑单个社区 $X_i$（大小为 $n_i$）。$D_i$ 和 $S_i$ 均由热迹定义。在 N 算子迭代中，$\Phi_i(M) = n_i \cdot D \cdot (1-S)$。

对任何 $M^{(k)}$ 在 $M^*$ 的某个邻域 $U$ 中，其更新步 $M^{(k+1)} = N(M^{(k)})$ 满足 $\|M^{(k+1)} - M^*\| \le \rho \|M^{(k)} - M^*\|$ 对某个 $\rho < 1$ 成立，即 $N$ 在 $U$ 上为压缩映射（Banach 不动点定理——$\rho(J_N(M^*)) < 1$ 保证 $N$ 在 $M^*$ 附近是指数收缩的，数值验证见 §B）。因此，一旦迭代进入 $U$，$M^{(k)} \to M^*$ 且 $\|\Delta M^{(k)}\| \to 0$。

在不动点处，$\Phi_i(M^*) = n_i \cdot D^* \cdot (1-S^*)$ 取定值。沿迭代轨迹，考虑差分：

$$\Delta\Phi^{(k)} = \Phi_i(M^{(k+1)}) - \Phi_i(M^{(k)})$$

在 $M^*$ 附近 Taylor 展开：

$$\Phi_i(M) = \Phi_i(M^*) + \nabla\Phi_i(M^*)^\top (M - M^*) + \frac{1}{2}(M - M^*)^\top \nabla^2\Phi_i(M^*)(M - M^*) + O(\|M-M^*\|^3)$$

在 N 迭代的每次更新中，$M - N(M)$ 的方向场 $v(M) = N(M) - M$ 已知。若 $v(M)$ 在 $U$ 内近似与 $-\nabla\Phi_i(M)$ 共线（梯度下降方向——在高维 N 算子中，各分量 $N_j(M) - M_j$ 的符号与 $-\partial\Phi_i/\partial M_j$ 一致。该性质源自 §3.1 耦合结构的构造：$D$ 上升意味着 $S$ 在自洽方程中推动 $D$ 增大，同时 $R$ 抑制 $D$——这恰好是 $\Phi_i = n_i D(1-S)$ 的梯度下降方向），则存在 $\alpha > 0$ 使 $\Delta\Phi^{(k)} \le -\alpha \|M^{(k)}-M^*\|^2$ 对充分大的 $k$ 成立。该梯度共线性在数值扫描中一致验证（100 组随机参数：$\langle v(M), -\nabla\Phi_i(M) \rangle > 0$ 在 $M^*$ 的某个邻域内始终成立，对应 $\dot\Phi < 0$）。

综合起来，$\Phi(M^{(k)})$ 在 $U$ 内单调递减至 $\Phi(M^*)$——即 $\dot\Phi \le 0$ 的离散类比。$\square$
...
约束流形上的梯度流。此方程在 Lipschitz 连续的右端项下具有局部存在唯一性（标准 ODE 理论：Perko 2001, §2.2）。在 $\mathcal{M}$ 的邻域内，$\Phi$ 具有 Lyapunov 函数的渐近性质：当系统进入 $M^*$ 的充分小邻域后（$F(M) \approx \mathbf{0}$ 使 $J_F \dot M \approx 0$），$\dot\Phi \le 0$（见定理 D）。全局 Lyapunov 性质的严格证明需 $J_F \dot M$ 项的非正性保证，该条件依赖于约束流形的全局几何性质，此处作为开放问题标注。
...
*证明纲要*。不动点 $N(M^*) = M^*$ 意味着 $F(M^*) = \mathbf{0}$，即 $M^* \in \mathcal{M}$。在该点 $\dot M = 0$，梯度流达到稳态。将 $M^{(k+1)} = N(M^{(k)})$ 重写为 $M^{(k+1)} - M^{(k)} = N(M^{(k)}) - M^{(k)}$，此即 $\dot M$ 的 Euler 离散化，其"速度场" $v(M) = N(M) - M$ 在 $M^*$ 处有 $v(M^*) = 0$ 且 Jacobian $J_v(M^*) = J_N(M^*) - I$。在非退化参数域（$\rho(J_N) < 1$），$M^*$ 是 N 迭代的指数吸引子；在梯度流中，由 $\dot E \le 0$ 保证 $M^*$ 是 $\Phi$ 的 Lyapunov 稳定平衡点。两套动力学在不动点 $M^*$ 处共享零速度场和指数收缩性质——N 迭代的压缩性（$\rho(J_N) < 1$）与梯度流在过阻尼极限下的衰减对应，数值验证（§B）确认二者在同一参数域内收敛到同一 $M^*$。$\square$
...
4. **热力学第二定律的信息类比**——$\Phi_i(M^{(k)})$ 沿 N 迭代单调递减（定理 D）是 $\varepsilon_5^{++}$ 与标准物理学最深层的连接点：信息传播就像热传导——总是从高耗散态流向低耗散态，直到达到平衡（不动点）。
