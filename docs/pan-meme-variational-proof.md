...
**九步。六个定理。一个变分原理。零 NP-hard。零自由参数。**
...
(ii) 深度单调性。$C_i \preceq C_j \Rightarrow A_i \subseteq A_j \Rightarrow n_i \le n_j$。因此 $\tau_i = n_i/n \le n_j/n = \tau_j$——子概念的热迹在更早的时间点上被评估。$G[X_i]$ 是 $G[X_j]$ 的导出子图。我们需要比较 $\Theta_i(\tau_i)/n_i$ 与 $\Theta_j(\tau_j)/n_j$。

对于**正则图**（所有顶点度数相等），归一化拉普拉斯等于缩放邻接拉普拉斯 $I - \frac{1}{d}A$，此时导出子图的特征值通过 Cauchy 交错定理与母图关联（Horn & Johnson 1985, Thm 4.3.17）：对 $\forall k \le n_i$，$\lambda_k^{(G[X_j])} \le \lambda_k^{(G[X_i])}$。因此 $\Theta_i(t) = \sum_{k=1}^{n_i} e^{-t\lambda_k^{(i)}} \le \Theta_j(t)$ 对所有 $t$ 成立。结合 $\tau_i \le \tau_j$ 和 $\Theta$ 的递减性：

$$\frac{\Theta_i(\tau_i)}{n_i} \le \frac{\Theta_j(\tau_i)}{n_i} \le \frac{\Theta_j(\tau_i)}{n_j} \le \frac{\Theta_j(\tau_j)}{n_j}$$

即 $D_i \le D_j$。$\square$

对于**一般图**，归一化拉普拉斯的导出子图关系不满足标准交错定理——这是图论中已知的开放问题（Chung 1997, §1.5 讨论）。深度单调性在一般图上的严格证明需额外条件（例如 $\epsilon$-正则性假设或局部树假设），目前**降级为物理猜想**，受以下支撑：(a) 正则图上的严格证明；(b) 随机图模型上 $\mathbb{E}[D_i] \le \mathbb{E}[D_j]$ 的概率证明（Molloy & Reed 2002 度序列框架）；(c) 数值实验验证（参见 §C）。
