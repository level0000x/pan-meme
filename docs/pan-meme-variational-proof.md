...
在不动点处 $\nabla\Phi_i(M^*) = \mathbf{0}$（自洽条件 $M^* = N(M^*)$ 的二阶推论——$\Phi$ 在 $M^*$ 处的梯度为零）。沿迭代轨迹，考虑差分：

$$\Delta\Phi^{(k)} = \Phi_i(M^{(k+1)}) - \Phi_i(M^{(k)})$$

在 $M^*$ 附近 Taylor 展开（$\nabla\Phi_i(M^*) = \mathbf{0}$）：

$$\Phi_i(M) \approx \Phi_i(M^*) + \frac{1}{2}(M - M^*)^\top \nabla^2\Phi_i(M^*)(M - M^*)$$

若 $\nabla^2\Phi_i(M^*) \succ 0$（Hessian 正定——$\Phi$ 在 $M^*$ 处为严格局部极小值，在数值扫描中一致验证），则存在常数 $c,C > 0$ 使 $c\|M - M^*\|^2 \le \Phi_i(M) - \Phi_i(M^*) \le C\|M - M^*\|^2$（邻域 $U$ 内）。结合压缩性 $\|M^{(k+1)}-M^*\| \le \rho\|M^{(k)}-M^*\|$：

$$\Delta\Phi^{(k)} = \Phi_i(M^{(k+1)}) - \Phi_i(M^{(k)}) \le C\rho^2\|M^{(k)}-M^*\|^2 - c\|M^{(k)}-M^*\|^2 = (C\rho^2 - c)\|M^{(k)}-M^*\|^2$$

当 $\rho < \sqrt{c/C}$ 时（高压缩率——标准参数下 $\rho \approx 0.547 \ll \sqrt{c/C}$，§B 确认），$\Delta\Phi^{(k)} \le 0$ 对充分大的 $k$ 成立，且 $\Delta\Phi^{(k)} \to 0$ 当 $k \to \infty$。综合起来 $\Phi(M^{(k)})$ 单调递减至 $\Phi(M^*)$，即 $\dot\Phi \le 0$ 的离散类比。$\square$
