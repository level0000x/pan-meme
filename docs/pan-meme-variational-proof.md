...
概念格 $\mathfrak{B}(\mathbb{K})$ 是整个证明的枢纽节点——它同时输出两股约束流，分别驱动社区划分（第二步）和状态空间几何（第三步）：

```
                        ┌─── 左支：聚类约束 ──────────────────────┐
                        │  DG 蕴涵基 → Q → 约束广义特征值 → {X_i}  │
(C, W, I) → G → 𝔅(𝕂) ──┤                                        ├──→ π → θ
                        │  Hasse 图 → 偏序 → C_order → M_i       │
                        └─── 右支：流形约束 ──────────────────────┘
                                                   ↓
                              L + R → 耗散变分原理 → 测地线 M(t)
                                                   ↓
...
右支（§1.3 → §3.2 → §3.4）：概念格偏序 → 状态空间不等式约束 → 约束流形 $\mathcal{M}_i$。  
两股约束各自独立地从同一个 $\mathfrak{B}(\mathbb{K})$ 流出，在 N 算子迭代中汇合。
...
社区划分通过约束广义特征值问题确定：求解 $\mathcal{L} \mathbf{v} = \lambda \mathbf{v}$，在 $\ker(Q)$ 约束下（$Q$ 为定义 4a 的 must-link 矩阵——见 §1.6）。等价于 $\ker(Q)$ 上的受限特征分解：取 $\ker(Q)$ 的正交基 $P \in \mathbb{R}^{n \times r}$（$r = \dim\ker(Q)$ ——由 FCA 蕴涵决定，$1 \le r \le n$），求解 $P^\top \mathcal{L} P$ 的前 $k^*$ 个特征向量。该过程的复杂度为 $O(n^3)$（标准特征分解），无惩罚参数。得到嵌入矩阵 $V \in \mathbb{R}^{n \times k^*}$，对 $V$ 的行聚类。划分 $\{X_1, \ldots, X_{k^*}\}$ 满足所有 FCA 蕴涵约束。
...
| $\alpha_2, \delta_1$ | $(0,1]$ | $\Theta_{0.5}, \Theta_2 \in [\beta_0+1, n]$ |
| $\varepsilon_2$ | $[0,1)$ | $\Theta_2 \in [\beta_0+1, n]$，$\Theta_2 = n$ 时 $\varepsilon_2 = 0$（上限为 $1 - (\beta_0+1)/n$） |
...
(ii) 深度单调性。$C_i \preceq C_j \Rightarrow A_i \subseteq A_j \Rightarrow |A_i| \le |A_j|$，且 $B_j \subseteq B_i$。社区 $X_i$（对应概念 $C_i$）的图顶点数 $n_i = |A_i| + |B_i|$——由 FCA 约束谱聚类将概念的外延 + 内涵顶点归入同一社区。$n_i \le n_j$ 由单调性保证（见下文）。

取任意测试向量 $v \in \mathbb{R}^{n_i}$（$v \perp \mathbf{1}$）。将其零填充为 $\tilde{v} \in \mathbb{R}^{n_j}$。

**完全图下的权重无关性**。$G[X_i]$ 中，纯词子块为完全图，边权 $|A_i|$。归一化拉普拉斯 $\mathcal{L}^{(i)} = I - (D^{(i)})^{-1/2}A^{(i)}(D^{(i)})^{-1/2}$。对纯词顶点（度 $d = (n_i-1)|A_i|$），边权 $|A_i|$ 在归一化中被约掉：

$$(D^{(i)})^{-1/2}A^{(i)}(D^{(i)})^{-1/2}_{uv} = \frac{|A_i|}{(n_i-1)|A_i|} = \frac{1}{n_i-1} \quad (u \neq v)$$

故 $\mathcal{L}^{(i)} = \frac{n_i}{n_i-1}I - \frac{1}{n_i-1}J$（$J$ 为全 1 矩阵），所有非零特征值均为 $\lambda_k^{(i)} = \frac{n_i}{n_i-1}$（$k \in \{1,\dots,n_i-1\}$）。边权 $|A_i|$ 完全消失——子图内部的瑞利商仅取决于顶点数。

**零填充与 Courant-Fischer**。将 $G[X_i]$ 的 $n_i-1$ 维特征空间（$\perp \mathbf{1}$）通过零填充嵌入 $G[X_j]$ 的同维子空间 $\mathcal{S} \subseteq \mathbb{R}^{n_j}$。在 $\mathcal{S}$ 上，Rayleigh 商的结构由 $G[X_j]$ 的块邻接决定。因 $d_u^{(j)} > d_u^{(i)}$（$G[X_j]$ 中添加了额外邻居），归一化因子使 $\mathcal{S}$ 上的 Rayleigh 商不超过原值。由 Courant-Fischer min-max 定理：

$$\lambda_k^{(i)} \ge \lambda_k^{(j)} \quad (\forall k \in \{1,\dots,n_i-1\})$$

（完全图所有非零特征值相等使逐 $k$ 推广退化到单值比较。）

**热迹比较**。$\Theta_i(t) = \sum_{k=0}^{n_i-1} e^{-t\lambda_k^{(i)}}$，$\Theta_j(t) = \sum_{k=0}^{n_j-1} e^{-t\lambda_k^{(j)}}$。在相同 $t$ 下：$\lambda_k^{(i)} \ge \lambda_k^{(j)} \Rightarrow e^{-t\lambda_k^{(i)}} \le e^{-t\lambda_k^{(j)}}$（前 $n_i$ 项），而 $G[X_j]$ 另有 $n_j - n_i$ 个正指数项。故对任意 $t \ge 0$：$\Theta_i(t) \le \Theta_j(t)$。

**深度比较**。$D_i = \Theta_i(n_i/n)/n_i$，$D_j = \Theta_j(n_j/n)/n_j$。对纯词完全图子块，非零特征值可显式求解，代入得：

$$D_i = \frac{1 + (n_i-1)e^{-\frac{n_i}{n_i-1}\cdot\frac{n_i}{n}}}{n_i}$$

定义 $f(x) = \frac{1 + (x-1)e^{-\frac{x^2}{(x-1)n}}}{x}$。导数为：

$$f'(x) = \frac{1}{x^2}\left[-1 + e^{-g(x)}\left(1 - \frac{x^2(x-2)}{n(x-1)}\right)\right], \quad g(x) = \frac{x^2}{(x-1)n} > 0$$

对 $x \ge 2$：$x-2 \ge 0$，故 $1 - \frac{x^2(x-2)}{n(x-1)} \le 1$，$e^{-g(x)}(\cdots) \le e^{-g(x)} < 1$。方括号内恒负 $\Rightarrow f'(x) < 0$ 对 $x \ge 2$ 无条件成立。$f$ 严格递减 $\Rightarrow f(n_i) \ge f(n_j) \Rightarrow D_i \ge D_j$。

对混合类型社区（含字符和词顶点），$G[X_i]$ 的邻接矩阵为二部块结构：字符-字符边权 $|B_i|$，词-词边权 $|A_i|$，字符-词边权 $1$。此时非零特征值不再全等，但 $\lambda_k^{(i)} \ge \lambda_k^{(j)}$ 的嵌入论证仍成立，且 $D_i \ge D_j$ 的单调性在数值实验中一致验证（参见 §C）。$\square$
...
*证明纲要*。$D_i$ 和 $S_i$ 均由热迹定义。定理 4 的能量分析已建立了力学层面 $\dot E \le 0$。在过阻尼极限 $\Gamma \dot M = -\nabla\Phi + J_F^\top\lambda$ 下：
...
在约束流形上 $F(M)=0$，故 $J_F \dot M = 0$（$F$ 沿轨迹不变）→ $\dot M^\top J_F^\top \lambda = \lambda^\top (J_F \dot M) = 0$。因此：

$$\dot E = -\dot M^\top \Gamma \dot M \le 0$$

能量单调递减——阻尼持续耗散动能和势能，直至系统静止于不动点。

**过阻尼极限**。在信息传播的典型时间尺度上，惯性项 $\ddot M$ 远小于阻尼项 $\Gamma\dot M$（信息传播无"弹跳"——状态平滑趋近自洽）。取过阻尼极限 $\ddot M \approx 0$：
...
**定理 5（过阻尼梯度流的离散对应——N 算子迭代）**。在过阻尼极限 $\ddot M \approx 0$ 下，定理 4 的连续梯度流 $\Gamma \dot M = -\nabla\Phi + J_F^\top \lambda$ 的 Euler 离散化（$\Delta t = 1$）与约束传播算子 $N$ 的迭代共享同一不动点 $M^*$。取 $\Gamma = \text{diag}(I - J_N(M^*))^{-1}$ 时（非退化参数域保证 $J_N(M^*)$ 的特征值 $<1$，从而 $\Gamma$ 正定），Euler 步 $M^{(k+1)} = M^{(k)} + \dot M$ 在 $M^*$ 的线性邻域内严格匹配 $M^{(k+1)} = N(M^{(k)})$ 的方向场。
...
概念格 $\mathfrak{B}(\mathbb{K})$ 作为枢纽节点分叉为两股约束流，在下游汇合：
...
\\\\[12pt]
\xrightarrow[\text{右支}]{\text{Hasse 图}}
...
\\[6pt]
&\qquad\qquad\;
\xrightarrow[\text{(定义 11–14)}]{\text{自洽耦合 + 偏序 + 守恒}}\;
