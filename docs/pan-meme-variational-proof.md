...
$P_2$ 三条件在不动点处形成内禀张力——没有任何模因能同时最大化三者（这是耦合结构的设计推论——增长力与衰减力的竞争在五个维度上同时进行，必然有至少一个维度被抑制，物理诠释见 §5.3）。不动点 $M^*$ 是约束流形上满足力平衡方程 $F(M)=0$ 的唯一自洽解——它不仅使各维度的增长力与衰减力平衡，而且状态 $M^{(k)}$ 和 $\Phi(M^{(k)})$ 均以指数速率 $\rho^k$ 弛豫至稳态（定理 D）。
