...
此方程组总有正解（中值定理：$\rho^* = 0$ 时 RHS $= \gamma_2/(\gamma_2+\delta_1) > 0$；$\rho^* \to \infty$ 时 RHS 有上界）。边界不动点的物理含义是"零深度、零韧度、满广度"——对应极度稀疏/碎片化的退化社区，恰与数值中 1/100 的双稳态情况对应（$\delta_2 = \lambda_{\max} \approx 2.0$ 触发）。
