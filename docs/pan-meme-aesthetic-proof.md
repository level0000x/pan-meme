...
**解释**。每个方程写作 $X = \frac{\sum(\text{增长力项})}{\sum(\text{增长力项}) + \sum(\text{衰减力项})}$。五条方程共 17 项——7 项增长力（$\alpha_2$, $\beta_1$, $\gamma_1$, $\gamma_2$, $\delta_1$, $\delta_2$, $\varepsilon_2$）和 10 项衰减力（$\alpha_1$ 出现两次，$\beta_2$ 出现两次，$\delta_1$, $\delta_2$, $\delta_3$ 各一次，$\varepsilon_1$, $\gamma_2$, $\delta_3$）。11 个参数无一遗漏（$\alpha_1$ 出现两次——作为 $D$ 的衰减和 $R$ 的衰减，编码"谱隙同时对深度和演化施加边界"的物理约束）。
...
**不动点的唯一性**：耦合方程组 $M = N(M)$ 是 5 个有理代数方程（每分量形如 $X = p_X(M) / q_X(M)$，$p_X, q_X$ 为多线性多项式）。对参数 11 元组的一般值（代数几何意义下——参数空间 $\mathbb{R}^{11}$ 的开稠子集），方程组的解集为有限离散点（零维代数簇）。在参数的 Zariski 开集外，解集唯一性由代数生成性保证。数值证据：100 组随机扫描中 99 组产生唯一不动点。剩余 1 组（$\delta_2 \approx 2.0$，即 $\lambda_{\max}$ 触及上界）出现双稳态，此时取典范初值 $M^{(0)} = (0.5,0.5,0.5,0.5,0.5)$ 的极限为选定不动点。此处理给出**算法意义上的唯一性**：固定参数，典范轨迹提供唯一确定的输出。$\square$
...
无分类。无不变量。无 Helmholtz。无 Jacobian。
...
**说明**。以下数值使用标准参数集 $(\alpha_{1,2} = 0.30, 0.70,\; \beta_{1,2} = 0.50, 0.40,\; \gamma_{1,2} = 0.60, 0.30,\; \delta_{1,2,3} = 0.10, 1.50, 0.20,\; \varepsilon_{1,2} = 0.25, 0.80)$——这些值源于典型 FCA 图（100 词、50 字、均匀共现分布）的归一化拉普拉斯特征谱，$\lambda_1 \approx 0.6$，$\lambda_{\max} \approx 1.6$，$t^* \approx 3.0$。这是示例参数，非通用最优。真实文本的参数需从 FCA 图的谱逐例计算。
...
- **$D \approx 0$ 可持吗？** $N_D = \alpha_2 S/(\alpha_2 S + \alpha_1 R)$。需 $S \ll R$。分两种子情形：(a) $R \approx 1$ ——被 $\varepsilon_1 > 0$ 绝对阻断（$N_R$ 分母含 $\varepsilon_1 > 0$ 项，故 $\sup_{M \in [0,1]^5} N_R(M) = (\delta_1+\delta_2)/(\delta_1+\delta_2+\alpha_1+\varepsilon_1) < 1$，$R$ 严格小于 1）。(b) $R$ 中等、$S$ 极小——此时 $N_D \approx 0$，但 $D \approx 0$ 使 $N_B$ 中 $\beta_2 D \approx 0$ → $N_B \approx 1$（若 $\rho > 0$）。$B \approx 1$ 通过 $N_S$ 的分母拉动 $S$（$\gamma_2 B$ 增大 → $N_S$ 下降），形成拉动 $S$ 回升的回路，破坏 $S \ll R$ 条件。**结论：$D \approx 0$ 不可自持。**
...
**综合**：$\partial[0,1]^5$ 上无 $N$ 的不动点，且零边界非吸收——五个坐标无一能永久停留在 0（反证法如上述逐条分析）。单步迭代可产生任意接近 0 的值（$N_D \to 0$ 当 $S \ll R$），但迭代的自正则化确保轨迹最终离开边界邻域。具体地说：经 $K \le 3$ 步后，所有坐标离开 $\varepsilon$-邻域 $[0, \varepsilon]$，不再返回——数值证据为所有 10000 测试点在 $k \ge 3$ 时 $M_j^{(k)} \ge 0.1$。
...
| ReasoningSoundness | 每步迭代保持在 $[0,1]^5$ 内（定理 4 的值域封闭性） |
