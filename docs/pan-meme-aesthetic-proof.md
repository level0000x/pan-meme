...
**注意**：$J_{\text{sym}}$ 在全局上不完全可积（由 $\partial J^{\text{sym}}_{D,B} / \partial R \neq \partial J^{\text{sym}}_{D,R} / \partial B$ 等条件验证，见附录 C），故 $\Phi$ 仅作为近平衡局部势。这恰好对应于"信息自由能是状态依赖的"，而非绝对量。
...
**引理 6（作用量有界性）**。若轨迹收敛至双曲平衡点 $M^*$（定理 6 情形 1），则 $S[\Gamma]$ 有限。

**证明**。$\|M(t)-M^*\| \le C e^{-\lambda t}$（Hartman-Grobman），$\|\dot{M}\| = O(e^{-\lambda t})$，$\Phi(M) = O(e^{-2\lambda t})$。积分收敛。$\square$
...
**$J_{\text{sym}}$ 不全局可积的验证**。取 $\partial J^{\text{sym}}_{1,2} / \partial R = \partial(\beta_2 B/2) / \partial R = 0$，而 $\partial J^{\text{sym}}_{1,4} / \partial B = \partial(\frac{1}{2}[- (D+R)\alpha_1 + \delta_1\rho]) / \partial B = 0$，两者相等。但验证 $J^{\text{sym}}_{0,2,3}$ 旋度：$\partial J^{\text{sym}}_{D,B} / \partial R = 0$，$\partial J^{\text{sym}}_{D,R} / \partial B = 0$——这对一致。关键的不可积性来自三元旋度 $\partial J^{\text{sym}}_{D,\rho} / \partial R \neq \partial J^{\text{sym}}_{D,R} / \partial \rho$：
