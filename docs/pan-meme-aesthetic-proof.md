...
**注**：$k^*=1$ 表示全图无显著分块结构（全图为一个社区），$k^* \approx n$ 表示图高度碎片化。退化情形：(a) 若最大谱隙不唯一（等间隔特征值），取最小的 $k$。(b) 若 $n \le 3$（无足够特征值计算间隙），取 $k^* = 1$。
...
**不动点的唯一性**：耦合方程组 $M = N(M)$ 是 5 个有理代数方程。通分后为 5 个多项式方程（最高 2 次）。数值扫描（100 组随机参数）表明：对参数的典型值，解集为单一内部不动点。剩余 1 组（$\delta_2 \approx 2.0$，即 $\lambda_{\max}$ 触及上界）出现双稳态（内部不动点 + 边界不动点 $(0,1,\rho^*,R^*,0)$），此时取典范初值 $M^{(0)} = (0.5,0.5,0.5,0.5,0.5)$ 的极限为选定不动点。此处理给出**算法意义上的唯一性**：固定参数，典范轨迹提供唯一确定的输出。$\square$
